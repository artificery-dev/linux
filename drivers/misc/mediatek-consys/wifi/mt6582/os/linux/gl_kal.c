    ...
    )
{
    ...
    do {
        ...
        i4Ret = request_firmware(&prGlueInfo->prFw,
                                 "mediatek/mt6582/WIFI_RAM_CODE_MT6582",
                                 prHifInfo->Dev);

        if (i4Ret) {
            printk(KERN_INFO DRV_NAME "firmware request failed %d\n", i4Ret);
            break;
        } else {
    ...
    )
{
    ...
    ASSERT(prGlueInfo);
    fgWasConnected = prGlueInfo->eParamMediaStateIndicated ==
    ...
    case WLAN_STATUS_MEDIA_CONNECT:
        ...
        if(prGlueInfo->fgIsRegistered == TRUE) {
            ...
            /* CFG80211 Indication */
            if(eStatus == WLAN_STATUS_MEDIA_CONNECT && !fgWasConnected) {
...
static void kalOsTimerCallback(struct timer_list *timer)
{
    P_GLUE_INFO_T prGlueInfo = from_timer(prGlueInfo, timer, tickfn);

    kalTimeoutHandler((unsigned long)prGlueInfo);
}
    ...
    )
{
    ...
    timer_setup(&(prGlueInfo->tickfn), kalOsTimerCallback, 0);
}
