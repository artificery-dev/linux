...
static const struct of_device_id mtk_wifi_of_match[] = {
    { .compatible = "mediatek,mt6582-wifi" },
    { }
};
MODULE_DEVICE_TABLE(of, mtk_wifi_of_match);

struct platform_driver MtkPltmAhbDriver = {
	.driver = {
		.name = "mt-wifi",
		.owner = THIS_MODULE,
		.of_match_table = mtk_wifi_of_match,
	},
    ...
    )
{
    ...
    /* Register AHB IRQ */
    if (HifAhbIrq < 0 || request_irq(HifAhbIrq,
                    HifAhbISR,
    ...
    if (request_irq(MT_GDMA2_IRQ_ID,
        ...
        printk("request irq %d fail!\n", MT_GDMA2_IRQ_ID);
        free_irq(HifAhbIrq, prNetDevice);
        return -1;
    }
#endif /* CONF_HIF_DMA_INT */
    ...
    )
{
    ...
    */
    HifAhbPDev = PDev;
    HifAhbIrq = platform_get_irq(PDev, 0);
    if (HifAhbIrq < 0)
        return HifAhbIrq;
