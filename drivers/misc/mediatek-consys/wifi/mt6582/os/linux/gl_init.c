...
//    prDev = &((struct sdio_func *) pvData)->dev; //samp
    ...
    prWdev->wiphy->regulatory_flags = REGULATORY_CUSTOM_REG;
	...
	if (wlan_if_changed) {
		prGlueInfo->prDevHandler = alloc_netdev_mq(sizeof(P_GLUE_INFO_T), NIC_INF_NAME_FOR_AP_MODE, NET_NAME_ENUM, ether_setup, CFG_MAX_TXQ_NUM);
	} else {
