...
scanReportBss2Cfg80211(
    IN P_ADAPTER_T prAdapter,
    IN ENUM_BSS_TYPE_T eBSSType,
    IN P_BSS_DESC_T prSpecificBssDesc)
{
    P_SCAN_INFO_T prScanInfo;
    P_BSS_DESC_T prBssDesc;
    ...
    if (prSpecificBssDesc) {
        if (prSpecificBssDesc->u2RawLength)
            kalIndicateBssInfo(prAdapter->prGlueInfo,
                    prSpecificBssDesc->aucRawBuf,
    ...
    prScanInfo = &prAdapter->rWifiVar.rScanInfo;
    LINK_FOR_EACH_ENTRY(prBssDesc, &prScanInfo->rBSSDescList,
            rLinkEntry, BSS_DESC_T) {
        if (prBssDesc->eBSSType != BSS_TYPE_INFRASTRUCTURE ||
            !prBssDesc->u2RawLength)
            continue;
        kalIndicateBssInfo(prAdapter->prGlueInfo,
                prBssDesc->aucRawBuf, prBssDesc->u2RawLength,
                prBssDesc->ucChannelNum,
                RCPI_TO_dBm(prBssDesc->ucRCPI));
        kalMemZero(prBssDesc->aucRawBuf, CFG_RAW_BUFFER_SIZE);
        prBssDesc->u2RawLength = 0;
