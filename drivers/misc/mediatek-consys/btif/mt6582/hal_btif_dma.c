...
/* Mainline port: resources come from the device tree, not fixed constants. */
void hal_btif_dma_set_resources(unsigned long apdma_base, int tx_irq, int rx_irq)
{
	mtk_btif_tx_dma.base = apdma_base + BTIF_TX_DMA_OFFSET;
	mtk_btif_rx_dma.base = apdma_base + BTIF_RX_DMA_OFFSET;
	mtk_btif_tx_dma_irq.irq_id = tx_irq;
	mtk_btif_rx_dma_irq.irq_id = rx_irq;
}
