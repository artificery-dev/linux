...
static int btif_ports_init(void);
...
/*
 * Mainline port: the BTIF block, its two AP_DMA virtual-FIFO channels, the
 * three interrupts and the two clock gates come from the device tree
 * ("mediatek,mt6582-btif"); the HAL's static tables are filled from here
 * and the DMA buffers are allocated against this device.
 */
struct device *btif_dma_dev;
...
static int mtk_btif_probe(struct platform_device *pdev)
{
	...
	struct resource *res;
	struct clk *clk;
	int irq_btif, irq_tx, irq_rx, ret;

	BTIF_TRC_FUNC("\n");
	consys_io_btif = devm_platform_ioremap_resource_byname(pdev, "btif");
	if (IS_ERR(consys_io_btif))
		return PTR_ERR(consys_io_btif);
	/* AP_DMA is shared with the i2c AP-DMA channels: map without claiming */
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "apdma");
	if (!res)
		return -ENODEV;
	consys_io_apdma = devm_ioremap(dev, res->start, resource_size(res));
	if (!consys_io_apdma)
		return -ENOMEM;
	irq_btif = platform_get_irq_byname(pdev, "btif");
	irq_tx = platform_get_irq_byname(pdev, "tx-dma");
	irq_rx = platform_get_irq_byname(pdev, "rx-dma");
	if (irq_btif < 0 || irq_tx < 0 || irq_rx < 0)
		return -ENODEV;
	clk = devm_clk_get(dev, "btif");
	if (IS_ERR(clk))
		return dev_err_probe(dev, PTR_ERR(clk), "btif clock\n");
	consys_clks[CONSYS_CLK_BTIF] = clk;
	clk = devm_clk_get(dev, "apdma");
	if (IS_ERR(clk))
		return dev_err_probe(dev, PTR_ERR(clk), "apdma clock\n");
	consys_clks[CONSYS_CLK_APDMA] = clk;
	ret = dma_set_mask_and_coherent(dev, DMA_BIT_MASK(32));
	if (ret)
		return ret;
	consys_irq_btif = irq_btif;
	consys_irq_btif_tx_dma = irq_tx;
	consys_irq_btif_rx_dma = irq_rx;
	hal_btif_set_resources((unsigned long)consys_io_btif, irq_btif);
	hal_btif_dma_set_resources((unsigned long)consys_io_apdma, irq_tx, irq_rx);
	btif_dma_dev = dev;
	ret = btif_ports_init();
	if (ret) {
		dev_err(dev, "BTIF port setup failed\n");
		return -EIO;
	}
	platform_set_drvdata(pdev, &g_btif[0]);
	dev_info(dev, "BTIF mapped, irqs %d/%d/%d\n", irq_btif, irq_tx, irq_rx);
	return 0;
}
...
/*vFIFO memory allocation*/
	p_vfifo->p_vir_addr = dma_alloc_coherent(btif_dma_dev,
						 p_vfifo->vfifo_size,
						 &p_vfifo->phy_addr, GFP_KERNEL);
	if (NULL == p_vfifo->p_vir_addr) {
...
/*free DMA memory if allocated successfully before*/
	if (NULL != p_vfifo->p_vir_addr) {
		dma_free_coherent(btif_dma_dev,
				  p_vfifo->vfifo_size,
				  p_vfifo->p_vir_addr, p_vfifo->phy_addr);
...
/*
 * Mainline port: the per-port software setup (which allocates the DMA
 * virtual FIFOs against the DT device and gates the clocks) runs from the
 * platform probe, not from module init, since the device is only known
 * once "mediatek,mt6582-btif" has probed.
 */
static int btif_ports_init(void)
{
	int i_ret = -1;
