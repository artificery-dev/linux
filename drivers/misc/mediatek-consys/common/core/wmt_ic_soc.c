...
static uint consys_ant_mode = 2;	/* 2 = use WMT_SOC.cfg */
module_param_named(ant_mode, consys_ant_mode, uint, 0644);
...
wmt_stp_init_coex (VOID)
{
    ...
    /* bring-up: 1 = BT and WiFi share one antenna (the arbiter decides who
     * transmits), 0 = separate antennas, no arbitration. Overrides
     * WMT_SOC.cfg so both can be tried without a reflash. */
    if (consys_ant_mode < 2)
        coex_table[COEX_WMT].cmd[5] = consys_ant_mode;
    printk(KERN_INFO "wmt: coex antenna mode %u\n", coex_table[COEX_WMT].cmd[5]);
