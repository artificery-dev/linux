...
INT32  wmt_ctrl_patch_search(P_WMT_CTRL_DATA pWmtCtrlData)
{
    ...
    /* mainline port: the patch table comes from the firmware images unless
     * that fails, in which case a userspace launcher may still answer */
    if (0 == consys_wmt_patch_search())
        return 0;
