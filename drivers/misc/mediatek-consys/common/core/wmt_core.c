    ...
    )
{
    ...
    for (i = 0; i < count; i++) {
        ...
        /* RESULT */
		/* bring-up: show what the firmware actually answered */
		printk(KERN_INFO "wmt init \"%s\": rx %d [%02X %02X %02X %02X %02X %02X] exp [%02X %02X %02X %02X %02X %02X]\n",
		       script[i].str, u4Res, evtBuf[0], evtBuf[1], evtBuf[2], evtBuf[3], evtBuf[4], evtBuf[5],
		       script[i].evt[0], script[i].evt[1], script[i].evt[2],
