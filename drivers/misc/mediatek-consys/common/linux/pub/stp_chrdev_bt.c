...
/* Tempo carries no Android NVRAM partition, so there is no factory address
 * record to read back; the controller keeps the address it powers up with. */
...
#if IS_ENABLED(CONFIG_MTK_CONSYS_BT)
bool stp_hci_channel_busy(void);
#else
static inline bool stp_hci_channel_busy(void) { return false; }
#endif

static int BT_open(struct inode *inode, struct file *file)
{
    /* hci0 and this node are the same channel; refuse rather than wedge it. */
    if (stp_hci_channel_busy()) {
        BT_ERR_FUNC("hci0 holds the STP Bluetooth channel; bring it down first\n");
        return -EBUSY;
    }
