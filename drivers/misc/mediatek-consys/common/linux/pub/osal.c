...
INT32 osal_dbg_assert_aee(const char *module, const char *detail_description){
    ...
    /* (mainline port: no AEE; the printk above is the report) */
    return 0;
}
...
/* mainline port: timer callbacks take the timer; recover the OSAL timer
 * and call the vendor handler with its data word as before. */
...
{
    timer_setup(&pTimer->timer, osal_timer_trampoline, 0);
    return 0;
}
...
INT32  osal_wake_lock_count(P_OSAL_WAKE_LOCK pLock)
{
    ...
    {
        count = pLock->wake_lock ? pLock->wake_lock->active : 0;
        return count;
    }
}
