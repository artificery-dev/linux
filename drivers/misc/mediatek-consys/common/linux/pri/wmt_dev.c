...
static ssize_t wmt_dev_dbg_write(struct file *file, const char __user *buffer,
...
static const struct proc_ops wmt_dbg_proc_ops = {
...
{
    /*
     * The vendor's write-only /proc/driver/wmt_dbg, restored on top of the
     * modern proc_ops. Arguments are hex: "7 <type> <0|1>" powers a function
     * on or off (type 0 = BT), "b <addr> <mask>" reads a chip register and
     * "c <addr> <value>" writes one, "9 <lvl>"/"a <lvl>" set the WMT and STP
     * log levels. Results land in the kernel log. Root-only, because "c"
     * writes arbitrary connectivity-chip registers.
     */
    gWmtDbgEntry = proc_create(WMT_DBG_PROCNAME, 0200, NULL, &wmt_dbg_proc_ops);
    if (gWmtDbgEntry == NULL) {
        WMT_ERR_FUNC("unable to create /proc/%s\n", WMT_DBG_PROCNAME);
        return -1;
    }
    WMT_INFO_FUNC("/proc/%s created\n", WMT_DBG_PROCNAME);
    return 0;
}
...
{
    /* mainline port: no /proc debug entry */
    return 0;
}
...
/*
 * Mainline port: patch and config files come from the firmware loader
 * (built into the kernel with CONFIG_EXTRA_FIRMWARE, or /lib/firmware),
 * under mediatek/mt6582/<basename>, instead of filp_open() on an Android
 * path. The buffer layout the callers expect is kept: padSzBuf bytes of
 * headroom, then the file from `offset`.
 */
static const char *wmt_dev_fw_name(const UCHAR *pName, char *buf, size_t sz)
{
    const char *base = strrchr(pName, '/');

    base = base ? base + 1 : (const char *)pName;
    snprintf(buf, sz, "mediatek/mt6582/%s", base);
    return buf;
}
    ...
    )
{
    ...
    *ppBufPtr = NULL;
    ret = request_firmware(&fw, wmt_dev_fw_name(pName, fwname, sizeof(fwname)), wmt_plat_get_dev());
    if (ret) {
        WMT_ERR_FUNC("firmware %s not found (%d)\n", fwname, ret);
        return -1;
    }
    ...
    memcpy(pBuf + padSzBuf, fw->data + offset, read_len);
    release_firmware(fw);
    *ppBufPtr = pBuf;
    WMT_INFO_FUNC("firmware %s: %d bytes\n", fwname, read_len);
    return read_len;
}
    ...
    )
{
    ...
    pfw = kzalloc(sizeof(osal_firmware), GFP_KERNEL);
    if (!pfw) {
...
MTK_WCN_BOOL wmt_dev_is_file_exist(UCHAR *pFileName)
{
    ...
    if (firmware_request_nowarn(&fw, wmt_dev_fw_name(pFileName, fwname, sizeof(fwname)), wmt_plat_get_dev()))
        return MTK_WCN_BOOL_FALSE;
    release_firmware(fw);
