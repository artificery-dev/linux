...
#define STP_DBG_AEE_EXP_API (0)	/* mainline port: no AEE */
enum {
...
static struct genl_family stp_dbg_gnl_family;	/* defined after its ops (mainline port) */
static struct genl_family __maybe_unused stp_dbg_gnl_family_old = {
    .hdrsize    = 0,
...
/* mainline port: ops are registered with the family */
static const struct genl_ops stp_dbg_gnl_ops[] = {
    { .cmd = STP_DBG_COMMAND_BIND, .doit = stp_dbg_nl_bind, },
    { .cmd = STP_DBG_COMMAND_RESET, .doit = stp_dbg_nl_reset, },
};
static struct genl_family stp_dbg_gnl_family = {
    ...
    .policy     = stp_dbg_genl_policy,
    .ops        = stp_dbg_gnl_ops,
    .n_ops      = ARRAY_SIZE(stp_dbg_gnl_ops),
    .module     = THIS_MODULE,
};
static void stp_dbg_nl_init(void)
{
