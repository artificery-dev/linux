...
#define CFG_WMT_WAKELOCK_SUPPORT 1	/* mainline port: struct wakeup_source */
...
wmt_plat_init (UINT32 co_clock_en)
{
    ...
    if (!wmtWakeLock)
        WMT_PLAT_ERR_FUNC("WMT-PLAT: wakeup_source_register(wmtFuncCtrl) failed\n");
    mutex_init(&gOsSLock);
    #endif
...
wmt_plat_deinit (VOID)
{
    ...
    wmtWakeLock = NULL;		/* so a later wake_lock_ctrl() cannot deref freed memory */
...
INT32 wmt_plat_wake_lock_ctrl(ENUM_WL_OP opId)
{
    ...
    /*
     * wmtWakeLock can be NULL: registration may have failed, or a func-ctrl
     * flow can reach here after wmt_plat_deinit() has released it (two BT
     * power-ons racing - a raw /dev/stpbt open against bluetoothd's hci0 -
     * used to fault here at ->active and take the radio down until reboot).
     * Guard every dereference rather than assume it is live.
     */
    if (!wmtWakeLock)
    {
        WMT_PLAT_WARN_FUNC("WMT-PLAT: wakelock unavailable, op(%d) counter(%d)\n", opId, counter);
        return 0;
    }
    if (WL_OP_GET == opId && counter == 1)
    {
        __pm_stay_awake(wmtWakeLock);
        WMT_PLAT_DBG_FUNC("WMT-PLAT: after wake_lock(%d), counter(%d)\n", wmtWakeLock->active, counter);
...
{
	return 0;	/* mainline port: no JTAG pin juggling */
}
#endif
#if CFG_WMT_DUMP_INT_STATUS
