...
static fm_s32 fm_ops_release(struct inode *inode, struct file *filp)
{
    ...
    /*
     * flush is called for every close of a duplicated descriptor, including
     * the close-on-exec copy inherited by a forked child.  The old vendor
     * implementation powered the receiver down there even while the
     * original descriptor remained open.  release is called once, when the
     * final reference to this file is gone, and is the correct lifetime for
     * the matching fm_open().
     */
    if (fm)
        fm_close(fm);
