...
struct fm_timer_ctx {
    ...
    struct fm_timer *owner;
};

static void fm_timer_dispatch(struct timer_list *timer)
{
    struct fm_timer_ctx *ctx = from_timer(ctx, timer, timer);

    if (ctx->owner->timeout_func)
        ctx->owner->timeout_func(ctx->owner->data);
}
...
{
    struct fm_timer_ctx *ctx = thiz->priv;
    struct timer_list *timerlist = &ctx->timer;
...
struct fm_timer* fm_timer_create(const fm_s8 *name) 
{
    ...
    if (!(ctx = fm_zalloc(sizeof(*ctx)))) {
        WCN_DBG(FM_ALT | MAIN, "fm_zalloc(fm_timer_ctx) -ENOMEM\n");
        fm_free(tmp);
        return NULL;
    }

    ctx->owner = tmp;
    timer_setup(&ctx->timer, fm_timer_dispatch, 0);
...
struct fm_work_ctx {
    ...
    struct fm_work *owner;
};

static void fm_work_dispatch(struct work_struct *work)
{
    struct fm_work_ctx *ctx = container_of(work, struct fm_work_ctx, work);

    if (ctx->owner->work_func)
        ctx->owner->work_func(ctx->owner->data);
}

static fm_s32 fm_work_init(struct fm_work *thiz, void (*work_func)(unsigned long data), unsigned long data)
{
    struct fm_work_ctx *ctx = thiz->priv;

    thiz->work_func = work_func;
    thiz->data = data;
    INIT_WORK(&ctx->work, fm_work_dispatch);
...
struct fm_work* fm_work_create(const fm_s8 *name) 
{
    ...
    if (!(ctx = fm_zalloc(sizeof(*ctx)))) {
        WCN_DBG(FM_ALT | MAIN, "fm_zalloc(fm_work_ctx) -ENOMEM\n");
        fm_free(my_work);
        return NULL;
    }
